...
@pytest.fixture
def arbiter():
    # use specific weights for clarity
...
def test_score_simulations_basic(arbiter, sample_sims):
    ...
    for sim in scored:
        ...
        expected = round(0.4*t1 + 0.4*t2 + 0.2*t3, 3)
...
def test_score_simulations_with_fatigue_suppression(arbiter):
    ...
    sim = scored[0]
    # Since fatigue_flag and reward_distortion >0.5, t3 suppressed to 0
    expected = round(0.4*0.3 + 0.4*0.6 + 0.2*0.0, 3)
    assert sim['final_score'] == pytest.approx(expected)
