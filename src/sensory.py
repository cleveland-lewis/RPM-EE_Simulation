...
class SensoryInputSystem:
    ...
    def _simulate_vision(self):
        if self.state == "asleep":
            return []
        return [{"modality": "vision", "intensity": random.uniform(0.1, 1.0), "duration": random.randint(1, 10)} for _
                in range(random.randint(0, 3))]

    def _simulate_hearing(self):
        if self.state == "asleep":
            return [{"modality": "hearing", "intensity": random.uniform(0.0, 0.3), "duration": random.randint(1, 5)}]
        return [{"modality": "hearing", "intensity": random.uniform(0.2, 1.0), "duration": random.randint(1, 10)} for _
                in range(random.randint(0, 2))]

    def _simulate_touch(self):
        if self.state == "asleep":
            return [{"modality": "touch", "intensity": random.uniform(0.0, 0.2), "duration": 1}]
        return [{"modality": "touch", "intensity": random.uniform(0.3, 1.0), "duration": random.randint(1, 4)} for _ in
                range(random.randint(1, 2))]

    def _simulate_smell(self):
        return [{"modality": "smell", "intensity": random.uniform(0.0, 0.6),
                 "duration": 3}] if random.random() < 0.3 else []

    def _simulate_taste(self):
        return [{"modality": "taste", "intensity": random.uniform(0.1, 0.7),
                 "duration": 2}] if random.random() < 0.1 else []
